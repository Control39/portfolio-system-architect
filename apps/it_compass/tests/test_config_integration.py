...
REPO_ROOT = Path(
    __file__
).parent.parent.parent.parent  # apps/it_compass/tests -> repo root
if str(REPO_ROOT) not in sys.path:


class TestItCompassConfigIntegration:
    """Тесты интеграции конфигурации It Compass"""
    ...
    def test_config_integration_module(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "config_integration",
            REPO_ROOT / "apps" / "it_compass" / "src" / "config_integration.py",
        )
        config_integration = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(config_integration)

        assert hasattr(config_integration, "ItCompassConfig")
    ...
    def test_get_config_singleton(self):
        ...
        assert (
            config1 is config2
        ), "get_config() должен возвращать один и тот же объект (singleton)"
