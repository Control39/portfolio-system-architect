...
class KnowledgeGraphConfig:
    """Обёртка для конфигурации Knowledge Graph через AI Config Manager"""
    ...
    def _load_local_config(self) -> None:
        ...
        local_config_path = REPO_ROOT / "apps" / "knowledge_graph" / "config" / "config.yaml"
    ...
    def get_config(self) -> dict[str, Any]:
        ...
        if self._config_manager:
            try:
                return self._config_manager.get_agent_config("knowledge_graph")
            except Exception:
...
_config_instance: KnowledgeGraphConfig | None = None
