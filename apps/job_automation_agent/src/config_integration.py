"""
Интеграция с AI Config Manager для Job Automation Agent
...
class JobAutomationAgentConfig:
    """Обёртка для конфигурации Job Automation Agent через AI Config Manager"""
    ...
    def _load_local_config(self) -> None:
        ...
        import yaml
        local_config_path = REPO_ROOT / "apps" / "job_automation_agent" / "config" / "config.yaml"
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        if self._config_manager:
            try:
                return self._config_manager.get_agent_config("job_automation_agent")
            except Exception:
...
_config_instance: Optional[JobAutomationAgentConfig] = None

def get_config() -> JobAutomationAgentConfig:
    """Получить глобальный экземпляр конфигурации"""
    global _config_instance
    if _config_instance is None:
        _config_instance = JobAutomationAgentConfig()
