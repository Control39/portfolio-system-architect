"""
Интеграция с AI Config Manager для Portfolio Organizer
...
class PortfolioOrganizerConfig:
    """Обёртка для конфигурации Portfolio Organizer через AI Config Manager"""
    ...
    def _load_local_config(self) -> None:
        ...
        import yaml
        local_config_path = REPO_ROOT / "apps" / "portfolio_organizer" / "config" / "config.yaml"
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        if self._config_manager:
            try:
                return self._config_manager.get_agent_config("portfolio_organizer")
            except Exception:
...
_config_instance: Optional[PortfolioOrganizerConfig] = None

def get_config() -> PortfolioOrganizerConfig:
    """Получить глобальный экземпляр конфигурации"""
    global _config_instance
    if _config_instance is None:
        _config_instance = PortfolioOrganizerConfig()
