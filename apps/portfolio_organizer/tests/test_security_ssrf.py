...
class TestSSRFProtection:
    ...
    def test_ssrf_blocked_gcp_metadata(self, client):
        ...
        response = client.get(
            "/api/portfolio/analyze", params={"url": "http://metadata.google.internal/computeMetadata/v1/"}
        )
        assert response.status_code == 400
        assert "SSRF" in response.json()["error"] or "Blocked" in response.json()["error"]
    ...
    def test_ssrf_blocked_azure_metadata(self, client):
        ...
        response = client.get(
            "/api/portfolio/analyze", params={"url": "http://169.254.169.254/metadata/instance?api-version=2021-02-01"}
        )
        assert response.status_code == 400
        assert "SSRF" in response.json()["error"] or "Blocked" in response.json()["error"]
...
class TestSSRFIntegration:
    ...
    def test_ssrf_with_url_encoded_payload(self, client):
        ...
        response = client.get(
            "/api/portfolio/analyze", params={"url": "http://%31%36%39%2e%32%35%34%2e%31%36%39%2e%32%35%34/metadata"}
        )
        assert response.status_code == 400
        assert "SSRF" in response.json()["error"] or "Blocked" in response.json()["error"]
