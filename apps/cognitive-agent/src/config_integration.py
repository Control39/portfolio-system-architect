"""
Интеграция с AI Config Manager для Cognitive-Agent
...
class CognitiveAgentConfig:
    ...
    def _load_local_config(self) -> None:
        ...
        import yaml
        local_config_path = REPO_ROOT / "apps" / "cognitive-agent" / "config" / "config.yaml"
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        if self._config_manager:
            try:
                return self._config_manager.get_agent_config("cognitive_agent")
            except Exception:
...
def get_config() -> CognitiveAgentConfig:
    ...
    if _config_instance is None:
        _config_instance = CognitiveAgentConfig()
