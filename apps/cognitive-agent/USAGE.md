...
python apps/cognitive-agent/tests/validation-test.py

# Или через скрипт запуска
python apps/cognitive-agent/launch-script.py --validate
```

### 2. Первый запуск
```bash
# Запуск агента с автономностью high
python apps/cognitive-agent/launch-script.py --autonomy=high

# Запуск с конкретным триггером
python apps/cognitive-agent/launch-script.py --trigger=project_open

# Запуск сканирования проекта
python apps/cognitive-agent/launch-script.py --scan
```
...
ls -la apps/cognitive-agent/reports/

# Проверка планов
ls -la apps/cognitive-agent/plans/
```
...
# Запустите агента
python apps/cognitive-agent/launch-script.py --trigger=project_open --autonomy=high
...
```bash
# Добавьте в crontab
0 9 * * * cd /path/to/project && python apps/cognitive-agent/launch-script.py --trigger=daily_maintenance
  ...
  cognitive-agent:
    ...
    steps:
      ...
      - name: Run Cognitive Agent
        run: |
          python apps/cognitive-agent/launch-script.py --trigger=ci_pipeline
          python apps/cognitive-agent/tests/validation-test.py
```
...
```yaml
# apps/cognitive-agent/config/integrations.yaml
github:
  enabled: true
  token: ${GITHUB_TOKEN}
  auto_pr_review: true
...
cat apps/cognitive-agent/logs/performance.csv
...
# Лог планировщика
tail -f apps/cognitive-agent/logs/planner.log

# Лог сканера
tail -f apps/cognitive-agent/logs/scanner.log

# Лог системы обучения
tail -f apps/cognitive-agent/logs/learning.log
```
...
pytest apps/cognitive-agent/tests/validation-test.py -v

# Запуск конкретных тестов
pytest apps/cognitive-agent/tests/validation-test.py::test_agent_structure -v
pytest apps/cognitive-agent/tests/validation-test.py::test_required_skills -v
pytest apps/cognitive-agent/tests/validation-test.py::test_configuration_files -v
```
...
python -m agents.learning.test --metrics-db=apps/cognitive-agent/data/trigger_metrics.db
...
python apps/cognitive-agent/launch-script.py --mode=autonomy-test --autonomy=high

# Тестирование проактивных триггеров
python apps/cognitive-agent/launch-script.py --trigger-test=project_open

# Тестирование механизма отката
python apps/cognitive-agent/launch-script.py --test-rollback --scenario=failure
```
...
cat apps/cognitive-agent/tests/latest_results.json
  ...
  test:
    ...
    steps:
      ...
      - name: Run validation tests
        run: python apps/cognitive-agent/tests/validation-test.py
      - name: Run functional tests
        run: pytest apps/cognitive-agent/tests/ -v
...
cat apps/cognitive-agent/config/agent-config.yaml | grep -A5 "triggers"

# Проверьте логи
tail -f apps/cognitive-agent/logs/cognitive_agent.log

# Запустите в режиме отладки
python apps/cognitive-agent/launch-script.py --debug --log-level=DEBUG
```
...
grep -i "error" apps/cognitive-agent/logs/cognitive_agent.log
...
# apps/cognitive-agent/skills/my-skill/SKILL.md
---
name: my-skill
...
# Логика скилла
# apps/cognitive-agent/skills/my-skill/__init__.py
def execute(context):
...
```bash
# Сбор данных
python -m agents.data.collect --output=apps/cognitive-agent/data/training/

# Обучение модели
python -m agents.models.train --data=apps/cognitive-agent/data/training/ --epochs=100
