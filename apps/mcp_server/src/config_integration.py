"""
Интеграция с AI Config Manager для Mcp Server
...
class McpServerConfig:
    """Обёртка для конфигурации Mcp Server через AI Config Manager"""
    ...
    def _load_local_config(self) -> None:
        ...
        local_config_path = REPO_ROOT / "apps" / "mcp_server" / "config" / "mcp-config.yaml"
    ...
    def get_config(self) -> dict[str, Any]:
        ...
        if self._config_manager:
            try:
                return self._config_manager.get_agent_config("mcp_server")
            except Exception:
...
_config_instance: McpServerConfig | None = None
