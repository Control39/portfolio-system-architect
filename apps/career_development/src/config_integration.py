"""
Интеграция с AI Config Manager для Career Development
...
class CareerDevelopmentConfig:
    """Обёртка для конфигурации Career Development через AI Config Manager"""
    ...
    def _load_local_config(self) -> None:
        ...
        local_config_path = REPO_ROOT / "apps" / "career_development" / "config" / "config.yaml"
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        if self._config_manager:
            try:
                return self._config_manager.get_agent_config("career_development")
            except Exception:
...
_config_instance: Optional[CareerDevelopmentConfig] = None


def get_config() -> CareerDevelopmentConfig:
    """Получить глобальный экземпляр конфигурации"""
    global _config_instance
    if _config_instance is None:
        _config_instance = CareerDevelopmentConfig()
