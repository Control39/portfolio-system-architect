...
def create_backup(file_path: Path, backup_dir: Path) -> bool:
    ...
    try:
        ...
        logging.exception(f"Ошибка при создании резервной копии {file_path}: {e}")
        return False
        ...
        try:
            ...
            if result["backup_created"]:
                try:
                    ...
                    logging.exception(f"Не удалось восстановить файл: {result['path']}")
...
def main():
    ...
    try:
        ...
        logging.exception(f"Ошибка при выполнении конвертации: {e}")
        exit(1)
