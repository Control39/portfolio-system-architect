...
from job_agent import process_request_sync, analyze_requirements, career_tracker, INTEGRATION_ENABLED
...
def test_integration():
    ...
    try:
        ...
        if "career_integration" in result:
            ...
            print(f"   Найдено соответствий: {result['career_integration']['matched_markers_count']}")
            ...
            if result['career_integration']['matched_markers_count'] > 0:
                ...
                if len(result['career_integration']['matched_marker_ids']) > 5:
                    print(f"   ... и ещё {len(result['career_integration']['matched_marker_ids']) - 5}")
            ...
            if new_markers != result['career_integration']['marked_as_completed']:
                print(f"⚠️ Количество новых маркеров ({new_markers}) не совпадает с отчетом интеграции ({result['career_integration']['marked_as_completed']})")
            else:
                print(f"✅ Количество новых маркеров ({new_markers}) совпадает с отчетом интеграции")
        ...
        if "career_integration" in result_unknown:
            if result_unknown['career_integration']['matched_markers_count'] == 0:
                ...
                print(f"⚠️ Найдены ложные соответствия: {result_unknown['career_integration']['matched_markers_count']}")
                return False
        else:
