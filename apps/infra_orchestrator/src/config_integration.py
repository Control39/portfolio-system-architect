"""
Интеграция с AI Config Manager для Infra Orchestrator
...
class InfraOrchestratorConfig:
    """Обёртка для конфигурации Infra Orchestrator через AI Config Manager"""
    ...
    def _load_local_config(self) -> None:
        ...
        import yaml
        local_config_path = REPO_ROOT / "apps" / "infra_orchestrator" / "config" / "appsettings.yaml"
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        if self._config_manager:
            try:
                return self._config_manager.get_agent_config("infra_orchestrator")
            except Exception:
...
_config_instance: Optional[InfraOrchestratorConfig] = None

def get_config() -> InfraOrchestratorConfig:
    """Получить глобальный экземпляр конфигурации"""
    global _config_instance
    if _config_instance is None:
        _config_instance = InfraOrchestratorConfig()
