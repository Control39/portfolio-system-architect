...
python apps/cognitive_agent/tests/validation-test.py

# Или через скрипт запуска
python apps/cognitive_agent/launch-script.py --validate
```

### 2. Первый запуск
```bash
# Запуск агента с автономностью high
python apps/cognitive_agent/launch-script.py --autonomy=high

# Запуск с конкретным триггером
python apps/cognitive_agent/launch-script.py --trigger=project_open

# Запуск сканирования проекта
python apps/cognitive_agent/launch-script.py --scan
```
...
ls -la apps/cognitive_agent/reports/

# Проверка планов
ls -la apps/cognitive_agent/plans/
```
...
# Запустите агента
python apps/cognitive_agent/launch-script.py --trigger=project_open --autonomy=high
...
```bash
# Добавьте в crontab
0 9 * * * cd /path/to/project && python apps/cognitive_agent/launch-script.py --trigger=daily_maintenance
  ...
  cognitive-agent:
    ...
    steps:
      ...
      - name: Run Cognitive Agent
        run: |
          python apps/cognitive_agent/launch-script.py --trigger=ci_pipeline
          python apps/cognitive_agent/tests/validation-test.py
```
...
cat apps/cognitive_agent/logs/performance.csv
...
# Лог планировщика
tail -f apps/cognitive_agent/logs/planner.log

# Лог сканера
tail -f apps/cognitive_agent/logs/scanner.log

# Лог системы обучения
tail -f apps/cognitive_agent/logs/learning.log
```
...
pytest apps/cognitive_agent/tests/validation-test.py -v

# Запуск конкретных тестов
pytest apps/cognitive_agent/tests/validation-test.py::test_agent_structure -v
pytest apps/cognitive_agent/tests/validation-test.py::test_required_skills -v
pytest apps/cognitive_agent/tests/validation-test.py::test_configuration_files -v
```
...
python -m agents.learning.test --metrics-db=apps/cognitive_agent/data/trigger_metrics.db
...
python apps/cognitive_agent/launch-script.py --mode=autonomy-test --autonomy=high

# Тестирование проактивных триггеров
python apps/cognitive_agent/launch-script.py --trigger-test=project_open

# Тестирование механизма отката
python apps/cognitive_agent/launch-script.py --test-rollback --scenario=failure
```
...
cat apps/cognitive_agent/tests/latest_results.json
  ...
  test:
    ...
    steps:
      ...
      - name: Run validation tests
        run: python apps/cognitive_agent/tests/validation-test.py
      - name: Run functional tests
        run: pytest apps/cognitive_agent/tests/ -v
...
cat apps/cognitive_agent/config/agent-config.yaml | grep -A5 "triggers"

# Проверьте логи
tail -f apps/cognitive_agent/logs/cognitive_agent.log

# Запустите в режиме отладки
python apps/cognitive_agent/launch-script.py --debug --log-level=DEBUG
```
...
grep -i "error" apps/cognitive_agent/logs/cognitive_agent.log
...
# apps/cognitive_agent/skills/my-skill/SKILL.md
---
name: my-skill
...
# Логика скилла
# apps/cognitive_agent/skills/my-skill/__init__.py
def execute(context):
...
```bash
# Сбор данных
python -m agents.data.collect --output=apps/cognitive_agent/data/training/

# Обучение модели
python -m agents.models.train --data=apps/cognitive_agent/data/training/ --epochs=100
