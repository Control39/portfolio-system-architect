...
class ReadmeGenerator:
    ...
    def _get_last_commit_date(self) -> str:
        ...
        try:
            result = subprocess.run(  # nosec B603, B607 - Trusted git command
                ["git", "log", "-1", "--format=%ci"], cwd=self.root, capture_output=True, text=True, check=True
            )
    ...
    def collect_adrs(self) -> list[dict[str, str]]:
        ...
        for file in sorted(adr_dir.glob("ADR-*.md")):
            ...
            if title_match:
                adrs.append(
                    {"number": title_match.group(1), "title": title_match.group(2).strip(), "filename": file.name}
                )
