#!/usr/bin/env python3
"""Unified healthcheck for Portfolio System Architect
Exports /health, /ready, /live endpoints for all services
"""

import subprocess
import sys
from datetime import datetime
from pathlib import Path

SERVICES = {
    "postgres": "localhost:5432",
    "pgadmin": "localhost:5050",
    "prometheus": "localhost:9090",
    "grafana": "localhost:3000",
    "career_development": "localhost:8001/health",
    "it_compass": "localhost:8501/health",
    "decision_engine": "localhost:8000/health",
}


def check_tcp(hostport: str) -> bool:
    host, port = hostport.split(":")
    result = subprocess.run(
        [
            "powershell",
            "-Command",
            f"Test-NetConnection -ComputerName {host} -Port {port} -InformationLevel Quiet",
        ],
        capture_output=True,
        text=True,
    )
    return result.returncode == 0


def main():
    healthy = True
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # Создаем папку для отчетов, если нет
    report_dir = Path("reports/audit")
    report_dir.mkdir(parents=True, exist_ok=True)

    report_path = report_dir / f"health_check_{timestamp}.json"

    print("🩺 Portfolio System Health Check")
    print("=" * 50)

    for name, endpoint in SERVICES.items():
        if check_tcp(endpoint.split("/")[0] if "/" in endpoint else endpoint):
            status = "🟢 UP"
        else:
            status = " DOWN"
            healthy = False
        print(f"{name:20} {status} {endpoint}")

    # Генерируем JSON отчет (упрощенно для примера)
    report_content = {"timestamp": timestamp, "status": "HEALTHY" if healthy else "DEGRADED", "services": SERVICES}

    # Записываем отчет в файл (в реальном проекте используй json.dump)
    report_path.write_text(str(report_content), encoding="utf-8")
    print(f"\n📝 Report saved to: {report_path}")

    if healthy:
        print("\n✅ ALL SYSTEMS HEALTHY")
        sys.exit(0)
    else:
        print("\n❌ CRITICAL SERVICES DOWN")
        sys.exit(1)


if __name__ == "__main__":
    main()
