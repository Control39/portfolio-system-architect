...
def demo_code_quality_analysis():
    ...
    print(
        f"  - Проблем безопасности: {summary.get('total_security_issues', 0)}")
    ...
    for tool, result in quality_report.get("results", {}).items():
        print(f"  - {tool}: {'Успешно' if result['success'] else 'Ошибка'}, "
              f"проблем: {result['issue_count']}")
    ...
    print(
        f"  - Приоритетных элементов: {len(improvement_plan['priority_items'])}")

    print("\n🎯 Рекомендации:")
    for i, rec in enumerate(improvement_plan['recommendations'], 1):
        print(
            f"  {i}. [{rec['priority'].upper()}] {rec['category']}: {rec['description']}")
...
def demo_project_scan_with_quality():
    ...
    if quality_results:
        ...
        print(
            f"  - Проблем безопасности: {summary.get('total_security_issues', 0)}")
        print(
            f"  - Покрытие тестами: {summary.get('coverage_percentage', 0)}%")
