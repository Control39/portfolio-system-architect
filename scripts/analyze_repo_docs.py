#!/usr/bin/env python3
"""
Analyze all markdown files in the repository and generate a structured report.

Usage:
    python analyze_repo_docs.py --output report.md
    python analyze_repo_docs.py --verbose
    python analyze_repo_docs.py --find-duplicates
"""
...
class TopicClassifier:
    ...
    TOPICS = {
        ...
        "user-guides": ["user", "usage", "configuration", "settings", "workflow", "task", "feature"],
        ...
        "agents": ["agent", "cognitive", "autonomous"],
    }
...
def find_duplicates(docs: List[Dict]) -> List[Tuple[Dict, Dict, float, List[str]]]:
...
def generate_file_tree(docs: List[Dict]) -> str:
...
def generate_report(docs: List[Dict], find_duplicates_flag: bool = False, verbose: bool = False) -> str:
    ...
    # Build report
    report = []
    report.append("# 📊 Анализ Markdown файлов репозитория")
...
def analyze_repo_docs(output_path: Optional[str] = None, find_duplicates: bool = False, verbose: bool = False) -> None:
    """Main function to analyze all markdown files in the repository."""

    repo_root = Path(__file__).parent.parent
    print(f"🔍 Анализ всех markdown файлов в: {repo_root}")

    # Collect all markdown files
    md_files = []
    for root, dirs, files in os.walk(repo_root):
        # Ignore hidden directories and git
        dirs[:] = [d for d in dirs if not d.startswith(".") and d not in {".git", ".ai_cache", ".gigacode"}]
    ...
    # Generate report
    report = generate_report(docs, find_duplicates_flag=find_duplicates, verbose=verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze all markdown files in the repository.")
    ...
    analyze_repo_docs(output_path=args.output, find_duplicates=args.find_duplicates, verbose=args.verbose)
