...
class ProjectAnalyzer:
    ...
    def _analyze_directory(self, path: Path):
        ...
        if depth > 6:
            self.stats["deep_paths"].append({"path": str(relative_path), "depth": depth})
        ...
        try:
            ...
            # Пропускаем директории, которые нельзя прочитать
            pass
    ...
    def generate_recommendations(self):
        ...
        if len(self.stats["duplicate_dirs"]) > 3:
            ...
            print("     → Создать единые 'skills', 'config', 'tools'")
            print("     → Использовать symlinks для обратной совместимости")

        if len(self.stats["suspicious_patterns"]) > 10:
            print("\n  3. 📦 Стандартизировать структуру")
            print("     → Перенести все конфиги в 'config/'")
            print("     → Перенести все логи в 'logs/'")
            print("     → Создать 'tools/' для инструментов анализа")
