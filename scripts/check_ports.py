        ...
        for port_mapping in ports:
            try:
                ...
                if container_port_int in port_map:
                    conflicts.append(
                        (container_port_int, port_map[container_port_int], service_name)
                    )
                else:
...
def main():
    ...
    # Отчёт (добавлен all_services)
    print_report(port_conflicts, route_conflicts, container_port_conflicts, routes, all_services)

    # Итог
    has_errors = bool(port_conflicts or route_conflicts or container_port_conflicts)
