...
def check_route_conflicts(routes: dict[str, dict[str, str]]) -> list[tuple[str, str, str]]:  # type: ignore[return-value]
    """
        ...
        for port_mapping in ports:
            try:
                ...
                if container_port in port_map:
                    conflicts.append((container_port, port_map[container_port], service_name))
                else:
                    port_map[container_port] = service_name
...
def print_report(
    ...
    print("\n" + "=" * 60)
    print("🔍 ОТЧЁТ ПРОВЕРКИ КОНФИГУРАЦИИ")
    print("=" * 60)
    ...
    if container_port_conflicts:
        print("❌ Обнаружены КОНФЛИКТЫ:")
        for port, svc1, svc2 in container_port_conflicts:  # type: ignore[assignment]
            print(f"   ⚠️  Порт {port}: {svc1} vs {svc2}")
    else:
    ...
    if routes:
        for service, info in sorted(routes.items()):
            print(f"   {service:30} → {info.get('route', 'N/A')}")
    else:
        print("   ⚠️  Маршруты Traefik не найдены")
...
def main():
    ...
    # Отчёт
    print_report(port_conflicts, route_conflicts, container_port_conflicts, routes)

    # Итог
    has_errors = port_conflicts or route_conflicts or container_port_conflicts
