...
async def test_yandex_gpt_connection() -> bool:
    ...
    try:
        ...
        try:
            ...
            # Создаем mock для тестирования без реального вызова API
            # В реальном тесте нужно раскомментировать следующий блок:
            """
            ...
            # Вместо реального вызова показываем mock
            print("  ⚠️  Режим mock: реальный запрос к API отключен для экономии токенов")
            print("     Для реального теста раскомментируйте блок кода в скрипте")
