...
class VectorStoreConfig(BaseModel):
    ...
    store_type: VectorStoreType = Field(
        default=VectorStoreType.CHROMA, description="Type of vector store"
    )
    persist_directory: str = Field(
        default="./chroma_db", description="Directory for persistent storage (ChromaDB)"
    )
    ...
    openai_api_key: str | None = Field(
        default=None, description="OpenAI API key (for OpenAI Vector Stores)"
    )
