> **Former ID:** ADR-003-ml-model-versioning-system **Former path:**
> `docs\architecture\decisions\ADR-003-ml-model-versioning-system.md` **Current ID:** ADR-005
...
Отсутствие четкой системы управления версиями ML-моделей приводит к потере воспроизводимости
экспериментов, сложностям с откатом к предыдущим версиям и проблемам с аудитом изменений. Необходимо
создать систему, обеспечивающую:
...
1. **Git LFS** - Использование Git LFS для хранения моделей
1. **MLflow** - Использование готового решения MLflow
1. **Пользовательская система** - Создание собственной системы управления версиями
...
1. **PostgreSQL** - хранение метаданных о моделях
1. **S3** - надежное хранилище для бинарных файлов моделей
1. **FastAPI** - REST API для взаимодействия с системой
1. **Docker** - контейнеризация для изоляции
1. **Kubernetes** - оркестрация и масштабирование
