...
**Дата:** 2026-06-13  
**Статус:** 🟢 Принято  
**Автор:** Kudelya Ekaterina  
**Контекст:** [ConfigManager как централизованная конфигурация]
...
2. **Отсутствие наблюдаемости** — нет метрик для Prometheus/Grafana
3. **Только YAML/JSON** — нельзя использовать TOML/ENV файлы
4. **Сложная отладка** — нет информации о времени загрузки, количестве ошибок
...
✅ **Наблюдаемость** — метрики для Prometheus/Grafana  
✅ **Гибкость** — поддержка TOML, JSON, YAML, ENV файлов  
✅ **Отладка** — время загрузки, количество ошибок в метриках

### Для эксплуатации

✅ **Мониторинг** — алерты на высокий latency загрузок  
✅ **Масштабируемость** — метрики для горизонтального масштабирования  
✅ **Трассировка** — можно связать с Jaeger/OpenTelemetry
