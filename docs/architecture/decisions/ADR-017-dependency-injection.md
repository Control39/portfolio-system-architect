> **Former ID:** ADR-018-dependency-injection **Former path:**
> `docs\architecture\decisions\ADR-018-dependency-injection.md` **Current ID:** ADR-017
...
**Дата:** 18 мая 2026 г. **Статус:** ✅ Принято **Автор:** Ekaterina Kudelya (Control39) **Решение:**
...
Проект `portfolio-system-architect` состоит из 15 микросервисов, которые должны взаимодействовать
друг с другом.
...
1. Возможность **замены реализации** без изменения кода потребителя
1. **Тестируемость** через mock-объекты
1. **Соблюдение границ** микросервисной архитектуры
...
**Причина выбора:** Соответствует принципам чистой архитектуры и микросервисов, обеспечивает
...
| Последствие | Описание | |-------------|----------| | **Zero Coupling** | Сервисы не зависят от
реализации друг друга | | **Pluggability** | Замена реализации занимает ~10 минут (править 1 файл) |
| **Testability** | Тесты на mock-объектах, мгновенные, без поднятия сервисов | |
**Maintainability** | Изменение API локализовано в адаптере | | **SOLID compliance** | Соблюдение
принципов D (Dependency Inversion) и I (Interface Segregation) |

### Негативные

| Последствие | Описание | |-------------|----------| | **Больше кода** | Нужно писать интерфейс +
адаптер (~50 строк кода) | | **Сложность для новичков** | Требуется понимание паттерна Dependency
Injection | | **Косвенная зависимость** | Оба сервиса зависят от `src/interfaces/` (но это
допустимо) |
...
1. ✅ Создать адаптер `CognitiveJobSearch` в `apps/infra_orchestrator/src/adapters/`
1. ✅ Обновить `job_automation_agent` для использования интерфейса
1. ✅ Удалить прямые импорты из `job_automation_agent`
1. ✅ Написать тесты на mock-объектах
1. ✅ Создать документацию в `docs/DEPENDENCY_INJECTION.md`
...
1. Рассмотреть внедрение **DI Container** для автоматического управления зависимостями:

   - `dependency_injector` (Python)
   - `injector` (Python)

1. Добавить конфигурацию выбора провайдера через переменные окружения:
