...
### 16 мая 2026 г. — Анализ legacy-папки `examples/`

**Выполненные задачи:**
1. **Полный анализ `examples/legacy/`** ✅
   - Проанализировано 28 файлов в 6 компонентах
   - Составлен детальный отчёт с рекомендациями

2. **Ключевые выводы:**
   - **70% устаревший код:** `api/`, `bot/`, `gateway/` (заменены новыми сервисами)
   - **30% актуальная документация:** `automation/README.md`
   - **Критично:** `ui/app.py` (актуальный код с SSRF защитой) требует переноса

3. **Создан отчёт:** ✅
   - `.reports/legacy-analysis-2026-05-15.md` — полный анализ с таблицей и рекомендациями

**Рекомендации:**
- [ ] **Приоритет 1:** Перенести `ui/app.py` → `apps/cognitive-agent/ui/` + добавить тесты
- [ ] **Приоритет 2:** Удалить `api/`, `bot/` (устаревшие)
- [ ] **Приоритет 3:** Архивировать `gateway/`, `terraform/` → `docs/examples/`
- [ ] **Приоритет 4:** Перенести `automation/README.md` → `docs/cases/`
