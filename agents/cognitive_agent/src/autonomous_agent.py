...
from agents.cognitive_agent.core.paths import get_agent_data_dir, get_repo_root
...
# ⭐ [LLM CLIENT] LLM client wrapper for AI Provider Manager
...
# ⭐ [FIX] Use project_path as repo root when available (for tests)
# get_repo_root() returns agents/cognitive_agent/, not C:/repo
def get_project_root(project_path: str) -> Path:
    """Get project root from project_path parameter"""
    return Path(project_path) if project_path else REPO_ROOT
...
class AutonomousCognitiveAgent(BaseCognitiveAgent):
    ...
    def __init__(self, project_path: str = None):
        ...
        # ⭐ [FIX] Use project_path as repo root when available
        self.project_root = get_project_root(project_path)
        ...
        root_prompts_dir = self.project_root / "prompts"  # Root-level prompts as source of truth

        # ⭐ [LLM CLIENT] Создать LLM client для генерации тестов
        ...
        logger.info(f"✅ Prompt engine initialized: {prompts_dir} (root: {root_prompts_dir})")
