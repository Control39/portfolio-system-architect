...
from ..chat_service.transports.self_host import ChatService as SelfChatService
from ..chat_service.transports.self_host import _InMemoryClientManager
...
async def test_build_self_transport():
    ...
    svc = build_chat_service(
        None, "localhost", 5001, store, DummyLogger(), transport_mode=TransportMode.SELF
    )
...
def test_build_webpubsub_missing_creds():
    ...
    with pytest.raises(RuntimeError):
        build_chat_service(
            None, "localhost", 5001, store, DummyLogger(), transport_mode=TransportMode.WEBPUBSUB
        )
