...
class WebPubSubChatService(ChatServiceBase):
    def __init__(
        ...
        *,
        ...
        if WebPubSubServiceClient is None:
            raise RuntimeError(
                "azure-messaging-webpubsubservice is not installed. Please `pip install azure-messaging-webpubsubservice`."
            )
        if endpoint:
            if credential is None:
                if DefaultAzureCredential is None:
                    raise RuntimeError(
                        "azure-identity is not installed. Please `pip install azure-identity` or pass a credential."
                    )
        ...
        class _SimpleClientManager:
            ...
            async def add_client(
                inner_self, connection_id: str, context: ClientConnectionContext, _transport: Any
            ) -> None:
    ...
    async def send_to_group(
        ...
        try:
            await self.room_store.record_room_event(
                room_id,
                {
                    "type": "message",
    ...
    def attach_flask_cloudevents(
        self, flask_app: Any, loop: asyncio.AbstractEventLoop, path: str = "/eventhandler"
    ) -> None:
        ...
        async def _wps_cloudevents() -> Union[Tuple[str, int], Tuple[str, int, Dict[str, str]]]:
            try:
                ...
                self.log.debug(
                    "CloudEvent received: type=%s userId=%s connectionId=%s",
                    ce_type,
                    user_id,
                    connection_id,
                )
                ...
                if ce_type.startswith("azure.webpubsub.user."):
                    ...
                    self.log.debug(
                        "User event received: event=%s connectionId=%s payload=%s",
                        event_name,
                        connection_id,
                        payload,
                    )
