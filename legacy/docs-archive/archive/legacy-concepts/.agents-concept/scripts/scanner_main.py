...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(".agents/logs/scanner.log"), logging.StreamHandler()],
)
...
class ProjectScanner:
                ...
                try:
                    with open(config_file, encoding="utf-8") as f:
                        ...
                        for db, keywords in config_patterns.items():
                            if any(keyword in content for keyword in keywords) and db.capitalize() not in databases:
                                databases.append(db.capitalize())
                except Exception:
                    continue  # nosec B110 B112
    ...
    def _check_hardcoded_secrets(self, path: Path) -> dict[str, Any]:
        """Проверка захардкоженных секретов

        Возвращает сводную статистику без раскрытия самих секретов.
        """
        ...
        for file_path in path.rglob("*.py"):
            if file_path.is_file():
                ...
                try:
                    with open(file_path, encoding="utf-8") as f:
                        content = f.read().lower()
                        found_keywords = [kw for kw in secret_keywords if kw in content]
                        if found_keywords:
                            # Сохраняем только путь и количество найденных ключевых слов
                            # без раскрытия самих значений
                            files_with_potential_secrets.append(
                                {
                                    "file": str(file_path.relative_to(path)),
                                    "keyword_count": len(found_keywords),
                                    "keywords_found": found_keywords,  # Только названия ключевых слов
                                }
                            )
                except Exception:
                    continue  # nosec B110 B112
        ...
        return {
            "total_files_checked": total_checks,
            "files_with_potential_secrets": len(files_with_potential_secrets),
            "potential_issues": files_with_potential_secrets[:10],  # Ограничиваем количество
            "recommendation": "Используйте переменные окружения или секреты-менеджеры",
        }
    ...
    def _check_duplication(self, path: Path) -> dict[str, Any]:
        ...
        return {"estimated_duplication": "low", "duplicated_files": [], "duplication_percentage": "0-5%"}
                        ...
                        for line in lines:
                            if line.strip().startswith("def "):
                                if function_lines > 50:
                                    smells.append(
                                        f"Длинная функция в {file_path.relative_to(path)}: {function_lines} строк"
                                    )
                                function_lines = 0
...
def main():
    ...
    try:
        ...
        # Безопасный вывод информации о безопасности
        security = results.get("security", {})
        secrets_info = security.get("secrets", {})
        if isinstance(secrets_info, dict):
            print("\nБезопасность:")
            print(f"  Проверено файлов: {secrets_info.get('total_files_checked', 0)}")
            print(f"  Потенциальных проблем: {secrets_info.get('files_with_potential_secrets', 0)}")
            if secrets_info.get("recommendation"):
                print(f"  Рекомендация: {secrets_info.get('recommendation')}")
        else:
            # Совместимость со старым форматом
            print("\nПроблем с секретами не найдено")

        scan_meta = results.get("scan_metadata", {})
        print(f"\nВремя сканирования: {scan_meta.get('scan_time', 0):.2f} сек")
        ...
        with open(status_file, "w", encoding="utf-8") as f:
            json.dump({"status": "error", "timestamp": time.time(), "error": str(e)}, f, indent=2)
