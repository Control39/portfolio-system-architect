...
def test_setup_logging():
    ...
    with patch("apps.assistant_orchestrator.main.logging") as mock_logging:
        from apps.assistant_orchestrator.main import setup_logging
...
def test_main_version_flag():
    ...
    with (
        ...
        patch("apps.assistant_orchestrator.main.sys") as mock_sys,
        patch("apps.assistant_orchestrator.main.__version__", "1.0.0", create=True),
    ):
...
def test_main_with_valid_root():
    ...
    with (
        patch("argparse.ArgumentParser", return_value=mock_parser),
        patch("apps.assistant_orchestrator.main.setup_logging"),
        patch("apps.assistant_orchestrator.core.analyzer.AssistantOrchestrator") as mock_orchestrator_class,
        patch("apps.assistant_orchestrator.core.reporter.Reporter") as mock_reporter_class,
        patch("pathlib.Path") as mock_path,
        patch("apps.assistant_orchestrator.main.logging"),
        patch("apps.assistant_orchestrator.main.sys") as mock_sys,
    ):
