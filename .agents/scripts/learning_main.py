...
class LearningSystem:
    ...
    def __init__(self, config_path: str = ".agents/config/learning.yaml"):
        ...
        # Создание директорий
        for directory in [self.data_dir, self.metrics_dir, self.models_dir, self.reports_dir]:
    ...
    def analyze_metrics(self, period_hours: int = 24) -> AgentMetrics:
        ...
        try:
            ...
            efficiency_score = (success_rate * 0.4 + avg_quality * 0.3 +
                              (1 - min(avg_duration / 3600, 1)) * 0.3)  # нормализованная оценка
        ...
        for task_type, stats in task_types.items():
            if stats["total"] >= 5:  # только для типов с достаточной статистикой
                ...
                if type_success_rate < 0.6:
                    suggestions.append(f"Улучшить выполнение задач типа '{task_type}': текущая успешность {type_success_rate:.1%}")
    ...
    def train_models(self) -> dict[str, Any]:
        ...
        try:
            if len(self.historical_metrics) < self.config["learning"]["min_samples_for_learning"]:
                logger.warning(f"Недостаточно данных для обучения. Требуется: {self.config['learning']['min_samples_for_learning']}, есть: {len(self.historical_metrics)}")
            ...
            return {"status": "success", "models_trained": len(training_results), "results": training_results}
    ...
    def _generate_human_readable_report(self, report: dict[str, Any], json_report_path: Path) -> None:
        ...
        try:
            ...
            with open(md_report_path, "w", encoding="utf-8") as f:
                ...
                f.write(f"- **Успешных задач:** {metrics['successful_tasks']} ({metrics['successful_tasks']/metrics['total_tasks']*100:.1f}%)\n")
