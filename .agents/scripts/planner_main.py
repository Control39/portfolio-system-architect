...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('.agents/logs/planner.log'),
...
class TaskPlanner:
    ...
    def prioritize_tasks(self) -> List[Dict[str, Any]]:
        ...
        weights = self.config.get("prioritization", {}).get("weights", {
            "urgency": 0.3,
            "importance": 0.4,
            "complexity": 0.1,
            "dependencies": 0.2
    ...
    try:
        ...
        with open(status_file, 'w', encoding='utf-8') as f:
            json.dump({
                ...
                "plan_id": timestamp if 'timestamp' in locals() else datetime.now().strftime("%Y%m%d_%H%M%S")
            }, f, indent=2)
