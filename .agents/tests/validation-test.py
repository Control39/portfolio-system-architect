...
class CognitiveAgentValidator:
    ...
    def validate_structure(self) -> dict[str, Any]:
        ...
        for dir_name in required_dirs:
            ...
            elif dir_name in ["logs", "scans", "reports", "backups", "cache"]:
        ...
        for skill_name in required_skills:
            ...
            if skill_path.exists():
                ...
                try:
                    ...
                    results["invalid"].append(f"{skill_name} (error: {e!s})")
            else:
        ...
        for config_file in required_configs:
            ...
            if config_path.exists():
                ...
                try:
                    ...
                    results["invalid_yaml"].append(f"{config_file} (error: {e!s})")
            else:
                # Проверяем, есть ли альтернативные имена
                alt_names = [f.stem + ext for ext in [".yaml", ".yml"] for f in config_dir.glob(f"*{ext}")]
                if config_file.replace(".yaml", "") in [name.replace(".yaml", "").replace(".yml", "") for name in alt_names]:
        ...
        for workflow_file in required_workflows:
            ...
            if workflow_path.exists():
                ...
                try:
                    ...
                    results["invalid"].append(f"{workflow_file} (error: {e!s})")
            else:
    ...
    def _is_test_passed(self, test_name: str, result: dict[str, Any]) -> bool:
        ...
        if test_name == "workflows":
            return len(result.get("valid", [])) > 0

        if test_name == "integrations":
        ...
        with open(md_file, "w", encoding="utf-8") as f:
            ...
            for test_name, test_result in validation_results["tests"].items():
                ...
                if test_name == "structure":
                    ...
                    f.write(f"- Файл интеграций: {'✅ Существует' if test_result.get('file_exists') else '❌ Отсутствует'}\n")
            ...
            if not issues:
                ...
                f.writelines(f"- {issue.replace('_', ' ').title()}\n" for issue in issues)

                f.write("\n**Действия для исправления:**\n")

                if "structure" in issues:
                    f.write("1. Создайте недостающие директории: `mkdir -p .agents/{config,workflows,logs,scans,reports}`\n")
            ...
            f.write("\n## Следующие шаги\n\n")
            f.write("1. Запустите тестовый рабочий процесс: `python -m agents.workflows.run --workflow=project-setup --test`\n")
...
def test_required_skills(validator):
    ...
    required_skills = ["cognitive-automation-agent", "project-scanner", "task-planner", "learning-system"]
